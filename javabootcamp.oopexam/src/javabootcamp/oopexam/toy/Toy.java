...
public abstract class Toy {
	...
	private void setToyAge() {
		Period period = Period.between(purchaseDay, LocalDate.now());
		toyAge = period.getYears();
	}
	...
	private boolean isToyNotBroken() {
		if(toyAge >= 1)
			return false;
		return true;
	}
	...
	private boolean checkValidAge(int minAllowed, int maxAllowed) {
		if(kidAge >= minAllowed && kidAge <= maxAllowed && isToyNotBroken())
			return true;
		return false;
	}
